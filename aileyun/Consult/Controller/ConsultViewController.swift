...
class ConsultViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let headRefresher = MJRefreshNormalHeader.init(refreshingTarget: self, refreshingAction: #selector(ConsultViewController.requestData))
        ...
        tableView.mj_header = headRefresher

        
        let footV = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(ConsultViewController.loadMoreData))
    ...
    func requestData(){
        ...
        currentPage = 1
        hasNext = true
        HttpRequestManager.shareIntance.HC_doctorList(hospitalId: hosId, pageNum: String.init(format: "%d", 1), callback: { [weak self](success, hasNext, arr, msg) in
            if success == true{
                self?.currentPage += 1
