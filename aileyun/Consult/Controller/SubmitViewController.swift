...
class SubmitViewController: BaseViewController {
    ...
    lazy var pickPhotoV : PhotoPickerView = {
        ...
        p.picBlock = { [weak self] arr in
            self?.uploadImage(image: arr.first)
        }
        return p
    }()
...
extension SubmitViewController {
    ...
    func uploadImage(image: UIImage?) {
        if let img = image {
            ...
            HttpRequestManager.shareIntance.HC_uploadSingleImg(img: img) { [weak self] (ret, url) in
                if ret == true {
                    self?.picArr.append(url)
...
//            }
            ...
            var uploadURL = ""
            if picArr.count > 0 {
                uploadURL = picArr.count == 1 ? picArr.first! : picArr.joined(separator: ",")
            }
            submitConsult(age: r.1, picS: uploadURL)
        }
    }
