...
class HttpRequestManager {
    ...
    // 获取余额
    func getBalance(cardSn: String, callback : @escaping (Bool, UserBalanceModel?)->()) {
        HttpClient.shareIntance.GET(USER_BALANCE, parameters: ["cardSn": cardSn]) { (result, ccb) in
            if ccb.success() {
                let arr = ccb.data as! [[String : Any]]
                if let dic = arr.first {
                    let model = JSONDeserializer<UserBalanceModel>.deserializeFrom(dict: dic)
                    callback(true, model)
                }else {
                    callback(false, nil)
                }
...
//            HCPrint(message: result)
    ...
    //MARK:
    //MARK: h5支付相关
    func HC_getOrderID(visitCard: String, price: String, callback : @escaping(Bool, String)->()) {
        let dic = NSDictionary.init(dictionary: ["visitCard" : visitCard, "price": price])
        HttpClient.shareIntance.POST(HC_ORDERID, parameters: dic) { (result, ccb) in
            if ccb.success() {
                if let dic = ccb.data as? [String : Any], let orderID = dic["orderId"] as? String {
                    callback(true, orderID)
                }else {
                    callback(false, "订单创建失败")
                }
            }else {
                callback(false, "订单创建失败")
            }
    ...
    func HC_getPayHrefH5(callback : @escaping(Bool, String)->()) {
        let url = HC_PAY_H5URL + "?keyCode=PAYMENT_H5_URL&token=\(UserManager.shareIntance.HCUser?.token ?? "")"
        HttpClient.shareIntance.GET(url, parameters: nil) { (result, ccb) in
            if ccb.success() {
                if let dic = ccb.data as? [String : Any], let url = dic["value"] as? String {
                    callback(true, url)
                }else {
                    callback(false, "服务器出错")
                }
            }else {
                callback(false, "服务器出错")
            }
