...
class HomeFunctionView: UIView {
    ...
    func actionWithModel(model : HomeFunctionModel){
        ...
        if model.name == "在线充值" {
            let rechargeVC = PrepareRechargeViewController.init(nibName: "PrepareRechargeViewController", bundle: Bundle.main)
            naviVC?.pushViewController(rechargeVC, animated: true)
            return
        }
