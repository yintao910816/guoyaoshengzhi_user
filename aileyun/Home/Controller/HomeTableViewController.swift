...
class HomeTableViewController: BaseViewController {
    ...
    lazy var naviBackV : UIView = {
        ...
        //        blurView.frame.size = CGSize(width: b.frame.width, height: b.frame.height)
...
//        tableV.tableFooterView = moreInformationV
    ...
    @objc func noticeDetail(){
        SVProgressHUD.show()
        let noticeModel = noticeV.modelArr![noticeV.row]
        if noticeModel.typeCom == "dynamic" {
            if let url = noticeModel.url, url.count > 0 && noticeModel.contentType?.intValue == 1{
                let webVC = WebViewController()
                webVC.url = url
